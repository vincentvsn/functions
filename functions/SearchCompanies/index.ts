...
const activityFunction: AzureFunction = async function (
  ...
  const hubspotClient = new hubspot.Client({
    accessToken: "pat-eu1-7b7968a5-d9f4-4960-8113-496d0a78624f", // To replace
  });

  try {
    console.log(context.bindingData.blabla);
    const apiResponse = await hubspotClient.crm.companies.searchApi.doSearch(
      context.bindingData.blabla
      );

      console.log("CONTEXT : ", context.bindingData);
